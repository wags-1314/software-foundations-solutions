...
Fixpoint is_in (n: nat) (l: bag) : bool :=
  match l with
  | nil => false
  | h :: t => if (n =? h) then true else is_in n t
end.

Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h :: t => if (is_in h s2) then subset t (remove_one h s2) else false
end.
...
Proof.
  intros v n b. simpl.
  intros H.
  assert (lemma: forall m, m =? m = true).
  - induction m.
    + reflexivity.
    + simpl. trivial.
  - rewrite lemma. rewrite H. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite IHl1. rewrite app_assoc. trivial.
Qed.
...
Proof.
  induction l.
  - trivial.
  - simpl. rewrite rev_app_distr. rewrite IHl. trivial.
Qed.
