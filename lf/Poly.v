...
(*
[x] = rev (cons x nil) => (rev nil) ++ (cons x nil)
*)
...
Proof.
  ...
  induction l.
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. rewrite <- app_assoc. simpl. reflexivity.
Qed.
...
Theorem app_nil_list: forall X (l : list X),
  [] ++ l = l.
Proof. simpl. reflexivity. Qed.

Theorem rev_nil: forall X: Type,
  @rev X [] = [].
Proof. simpl. reflexivity. Qed.

Theorem rev_cons: forall (X: Type) (n: X) (l: list X),
  rev(n :: l) = rev(l) ++ [n].
Proof.
  intros.
  induction l.
...
Proof.
  intros.
  induction l1.
  - rewrite app_nil_list. rewrite rev_nil. rewrite app_nil_r. simpl. reflexivity.
...
rev [x] = rev (cons x nil) => (rev nil) ++ (cons x nil) = nil ++ [x] = [x]
...
Compute (combine [1;5] [false;true;true;true]).
...
Fixpoint firsts {X Y : Type} (l : list (X*Y)) : (list X) :=
  match l with
  | nil => nil
  | h :: t => (fst h) :: firsts t
end.

Fixpoint seconds {X Y : Type} (l : list (X*Y)) : (list Y) :=
  match l with
  | nil => nil
  | h :: t => (snd h) :: seconds t
end.
...
Definition func : Set -> Set := (fun n => n).
...
Definition gtb (a b : nat) : bool :=
  negb (leb a b).

Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun l => (gtb l 7) && (evenb l)) l.
...
Definition compose {X Y Z : Type} (f : X -> Y) (g: Y -> Z) :=
  (fun n => g (f n)).
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  ( (filter test l), (filter (compose test negb) l)).
...
Lemma l1 : forall (X : Type) (l : list X) (x : X),
...
(*
  ...
  [1,2,3,4]
  f n => n + 1
  (fold func [1,2,3,4] nil)
  1 :: [2,3,4]
  func 1 (fold func [2,3,4] nil)
  (f 1) :: (fold func [2,3,4] nil)
*)
...
x : X
l : list X
IHl : fold_map f l = map f l
______________________________________(1/1)
fold_map f (x :: l) = f x :: map f l
*)
(* FILL IN HERE *)

Lemma l2 : forall (X Y : Type) (f: X -> Y) (l: list X) (x : X),
...
Theorem corr_of_fold_map : forall (X Y : Type) (f: X -> Y) (l: list X),
  fold_map f l = map f l.
Proof.
  intros.
  induction l.
  - simpl. reflexivity.
  - simpl. rewrite l2. rewrite IHl. simpl. reflexivity.
Qed.
