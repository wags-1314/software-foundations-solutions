...
Proof.
  ...
  - (* n = S n' *) intros eq. destruct m as [| m'] eqn:E.
    + (* m = O *) discriminate eq.
    + (* m = S m' *) simpl in eq. injection eq as eq1.
...
(** At this point, since we are in the second branch of the [destruct
    ...
    [IHn'] gives us exactly what we need to finish the proof. *)
      ...
      apply IHn'. simpl in eq. injection eq as eq1. apply eq1. Qed.
...
Proof.
  ...
  - destruct m as [|m'].
    + simpl. discriminate.
    + simpl. intros. apply IHn' in H. f_equal. apply H.
Qed.
...
Lemma length_eq_O_implies_nil: forall (X: Type) (l : list X), 
  length l = 0 -> l = nil.
Proof.
  intros.
  induction l.
...
Proof.
  ...
  - destruct n as [| n'].
    + simpl. discriminate.
    + simpl. intros H. injection H as H1. apply IHl in H1. apply H1.
Qed.
...
Proof.
  ...
  induction l as [| p l'].
  - simpl. intros. injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl. reflexivity.
  - intros. destruct p as [x y]. simpl in H. destruct (split l') as [p q]. injection H as H1 H2.
...
Proof.
  ...
  - simpl. intros l2 H. symmetry in H.
    apply length_eq_O_implies_nil in H. rewrite H. reflexivity.
  - intros. simpl in H. destruct l2 as [| x2 l2].
    + simpl in H. discriminate H.
    + simpl in H. injection H. intros H1. apply IHl1 in H1. simpl. rewrite H1. reflexivity.
Qed.
...
Proof.
  ...
  - simpl in H. discriminate H.
  - simpl in H. destruct (test x') eqn:H0.
...
Lemma DeMorgansLaw1 : forall a b,
...
Proof.
  ...
  - simpl. unfold existsb'. simpl. rewrite DeMorgansLaw1. rewrite negb_involutive.
    f_equal. unfold existsb' in IHl'. apply IHl'.
 Qed.
